...
#define BWR_420_Len 50
uint8_t LUT_bwr_420_part[] = {
...
#define EPD_BWR_420_test_pattern 0xA5
 uint8_t EPD_BWR_420_detect(void)
{
    ...
    {
        EPD_WriteData(EPD_BWR_420_test_pattern);
    }
    EPD_WriteCmd(0x33);
    for (i = 0; i < 153; i++)
    {
        if (EPD_SPI_read() != EPD_BWR_420_test_pattern)
            return 0;
    }
    return 1;
}
 ...
 uint8_t EPD_BWR_420_Display_start(uint8_t full_or_partial)
{
    ...
    // Set RAM X- Address Start/End 54
    EPD_WriteCmd(0x44);
    EPD_WriteData(0x00);
    EPD_WriteData(0x36); //0F

    // Set RAM Y- Address Start/End 296
    EPD_WriteCmd(0x45);
    EPD_WriteData(0x00);
    EPD_WriteData(0x00);
    EPD_WriteData(0x30); //
 ...
 uint8_t EPD_BWR_420_Display(unsigned char *image, int size, uint8_t full_or_partial)
{
    ...
    {
        EPD_WriteCmd(0x32);
        for (i = 0; i < sizeof(LUT_bwr_420_part); i++)
        {
            EPD_WriteData(LUT_bwr_420_part[i]);
        }
    }
