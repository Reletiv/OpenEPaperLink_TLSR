 ...
 uint8_t EPD_BWR_266_read_temp(void)
{
    ...
    EPD_WriteCmd(0x11);
    EPD_WriteData(0x01); // EPD_WriteData(0x01);

    // Set RAM X- Address Start/End
    EPD_WriteCmd(0x44);
    EPD_WriteData(0x00);
    EPD_WriteData(((152-1) >> 3) & 0x1f);  // EPD_WriteData(0x0F);

    // Set RAM Y- Address Start/End
    //EPD_WriteCmd(0x45);
    //EPD_WriteData(0); // EPD_WriteData(0x28);
    //EPD_WriteData(0); // EPD_WriteData(0x01);
    //EPD_WriteData((296&0xff)); // EPD_WriteData(0x2E); 
	//EPD_WriteData((296&0x100)>>8); // EPD_WriteData(0x00);

    // Set RAM Y- Address Start/End
    EPD_WriteCmd(0x45);
    EPD_WriteData((296 & 0xFF)); // Start from maximum Y-coordinate
    EPD_WriteData((296 >> 8) & 0xFF); // EPD_WriteData(0x00);
    EPD_WriteData(0); // Start from maximum Y-coordinate
    EPD_WriteData(0); // Start from maximum Y-coordinate
...
{    
    ...
    // Set RAM Y address
    EPD_WriteCmd(0x4F);
    EPD_WriteData((296&0xff)); 
	EPD_WriteData((296&0x100)>>8); 
    //EPD_WriteData(0x00); // 0x28
    //EPD_WriteData(0x00); // 0x01
 ...
 uint8_t EPD_BWR_266_Display_start(uint8_t full_or_partial)
{
    ...
    // Set RAM X- Address Start/End
    EPD_WriteCmd(0x44);
    EPD_WriteData(0x01); // 00
    EPD_WriteData((((152-1) >> 3) & 0x1f)+1);  // EPD_WriteData(0x0F);
